...
namespace QLDSV_TC.views
{
    public partial class frmClass : DevExpress.XtraEditors.XtraForm
    {
        ...
        private bool IsKeToanBranch()
        {
            try
            {
                // Ưu tiên check theo server nếu bạn đã quy ước MSSQLSERVER5 là KT
                var server = (Program.servername ?? "").ToUpperInvariant();
                if (server.Contains("MSSQLSERVER5")) return true;

                // Hoặc check theo tên hiển thị trong combobox
                var drv = cbKhoa.SelectedItem as DataRowView;
                var tenPhong = drv?["TENPHONG"]?.ToString() ?? "";
                return tenPhong.IndexOf("Kế Toán", StringComparison.OrdinalIgnoreCase) >= 0
                    || tenPhong.IndexOf("Ke Toan", StringComparison.OrdinalIgnoreCase) >= 0;
            }
            catch { return false; }
        }
        ...
        private void ReloadData()
        {
            try
            {
                if (IsKeToanBranch())
                {
                    // Chi nhánh Kế Toán không có các bảng đào tạo => xóa hiển thị hiện tại, không fill
                    if (this.qLDSV_TCDataSet.LOP != null) this.qLDSV_TCDataSet.LOP.Clear();
                    if (this.qLDSV_TCDataSet.SINHVIEN != null) this.qLDSV_TCDataSet.SINHVIEN.Clear();
                    if (this.qLDSV_TCDataSet.DANGKY != null) this.qLDSV_TCDataSet.DANGKY.Clear();

                    // Tùy bạn muốn: hiện thông báo nhẹ cho người dùng
                    // MessageBox.Show("Chi nhánh Kế Toán không có dữ liệu Lớp/Sinh viên/Đăng ký. Vui lòng mở form Học phí để xem dữ liệu.");

                    return;
                }

                // Các chi nhánh đào tạo (CNTT/VT): fill như cũ
                this.lOPTableAdapter.Connection.ConnectionString = Program.connstr;
                this.lOPTableAdapter.Fill(this.qLDSV_TCDataSet.LOP);
            ...
            {
                MessageBox.Show("Không load được dữ liệu từ server mới.\n" + ex.Message);
            }
        }
        ...
        {
            if (cbKhoa.SelectedValue == null || cbKhoa.SelectedValue.ToString() == "System.Data.DataRowView")
                return;
            ...
            if (cbKhoa.SelectedIndex != Program.mPhongBan)
            {
                ...
                Program.pass = Program.remotePass;    // 18
            }
            else
            {
